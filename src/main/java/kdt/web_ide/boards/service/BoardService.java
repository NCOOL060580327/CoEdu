...
public class BoardService {
    ...
    // 게시판 수정
    @Transactional
    public BoardResponseDto updateBoard(BoardUpdateRequestDto requestDto, Long boardId,Member member) {
    ...
    // 게시판 인원 초대
    @Transactional
    public BoardUserResponseDto inviteMember(BoardUserInviteRequestDto requestDto, Long boardId,Member currentMember) {
    ...
    public void removeMember(Long boardId, Long memberId, Member currentMember) {
        ...
        // 삭제 권한 확인 (리더만 가능)
        BoardUser leader = boardUserRepository.findByMemberAndBoardAndIsLeaderTrue(currentMember, board)
                .orElseThrow(() -> new CustomException(ErrorCode.NO_PERMISSION));
        ...
        BoardUser boardUser = boardUserRepository.findByMemberAndBoard(memberToRemove, board)
